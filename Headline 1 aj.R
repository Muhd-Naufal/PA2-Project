...
rapply( Punggoldf, f=function(x) ifelse(is.nan(x),0,x), how="replace")

class(Punggoldf$) <- "dollar"
format.dollar <- function(x, ...) paste0("$", unclass(x))

for (Punggol2$year = c(2007:2017), Punggol2$flat_type == "2 ROOM"){
  print(Punggol2$resale_price)
}

h <- if (Punggol2$year = c(2007:2017) & Punggol2$flat_type == "2 ROOM") {
  print(nrow(Punggol2$resale_price)
}

g <- filter(Punggol2, year == c(2007:2017), flat_type == "2 ROOM")
g
