...
AMK <- filter(hdb3, town == "ANG MO KIO")
AMKMax <- max(AMK$resale_price)

BDK <- filter(hdb3, town == "BEDOK")
BDKMax <- max(BDK$resale_price)

BSN <- filter(hdb3, town == "BISHAN")
BSNMax <- max(BSN$resale_price)

BB <- filter(hdb3, town == "BUKIT BATOK")
BBMax <- max(BB$resale_price)

BM <- filter(hdb3, town == "BUKIT MERAH")
BMMax <- max(BM$resale_price)

BP <- filter(hdb3, town == "BUKIT PANJANG")
BPMax <- max(BP $resale_price)

BT <- filter(hdb3, town == "BUKIT TIMAH")
BTMax <- max(BT$resale_price)

AMK <- filter(hdb3, town == "ANG MO KIO")
AMKMax <- max(AMK$resale_price)

CA <- filter(hdb3, town == "CENTRAL AREA")
CAMax <- max(CA$resale_price)

CCK <- filter(hdb3, town == "CHOA CHU KANG")
CCKMax <- max(CCK$resale_price)

CMT  <- filter(hdb3, town == "CLEMENTI")
CMTMAX <- max(CMT$resale_price)

GY    <- filter(hdb3, town == "GEYLANG")
GYMAX<- max(GY$resale_price)

HG <- filter(hdb3, town == "HOUGANG")
HGMAX<- max(HG$resale_price)

JE <- filter(hdb3, town == "JURONG EAST")
JEMAX<- max(JE$resale_price)

JW<- filter(hdb3, town == "JURONG WEST")
JWMAX<- max(JW$resale_price)

KW <- filter(hdb3, town == "KALLANG/WHAMPOA")
KWMAX<- max(KW$resale_price)

MP<- filter(hdb3, town == "MARINE PARADE")
MPMAX<- max(MP$resale_price)

QNT<- filter(hdb3, town == "QUEENSTOWN")
QNTMAX<- max(QNT$resale_price)

SK<- filter(hdb3, town == "SENGKANG")
SKMAX<- max(SK$resale_price)

SSR<- filter(hdb3, town == "SERANGOON")
SSRMAX<- max(SSR$resale_price)

TP<- filter(hdb3, town == "TAMPINES")
TP<- max(TP$resale_price)

TYH <- filter(hdb3, town == "TOA PAYOH")
TYHMAX<- max(TYH$resale_price)

WD<- filter(hdb3, town == "WOODLANDS")
WDMAX<- max(WD$resale_price)

YYN<- filter(hdb3, town == "YISHUN")
YYNMAX<- max(YYN$resale_price)

LCK<- filter(hdb3, town == "LIM CHU KANG")
LCKMAX<- max(LCK$resale_price)

SBW<- filter(hdb3, town == "SEMBAWANG")
SBWMAX<- max(SBW$resale_price)

PR<- filter(hdb3, town == "PASIR RIS")
PRMAX<- max(PR$resale_price)

PPL<- filter(hdb3, town == "PUNGGOL")
PPLMAX<- max(PPL$resale_price)
